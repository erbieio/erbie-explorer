...
import {transactionDetail,transaction,recycle_tx} from "../../api/request_data/Trade_request";
...
class TradeDetail extends React.Component {
    ...
    componentDidMount() {
        ...
        if (this.props.location.state) {
            window.sessionStorage.setItem('Trastate', this.props.location.state);
        ...
        this.commonFunc = () => {
            ...
            this.handleChange = (value,index1,index,index2) => {
                ...
                if (index2 === "Hex") {
                     this.state.dataLog.map((item,ind) => {
                        let detail = [];
                        ind === index ? item.topics.forEach((item1,ind1) => {
                            ind1 === index1 ? detail.push(JSON.stringify({ type : true, value:JSON.parse(item1).value}) )  : detail.push(item1)
                        }): this.state.dataLog
                        rea.forEach(reaitem => {
                  ...
                  // val = JSON.stringify(JSON.parse(this.state.dataLog[index].topics[index1]).type = true)
                     ...
                     this.state.dataLog.forEach((item,ind) => {
                         let detail = [];
                        ind === index ? item.topics.forEach((item1,ind1) => {
                            ind1 === index1 ? detail.push(JSON.stringify({ type : false, value:JSON.parse(item1).value}) )  : detail.push(item1)
                        }): this.state.dataLog
                         rea.forEach(reaitem => {
                             return  reaitem.topics = detail
                         })
            ...
            (async () => {
                ...
                if(res) {
                    res.forEach(item => {
                        ...
                        item.topics.forEach(value => {
                           val.push(JSON.stringify({type:true,value: value}))
                        })
                        item.topics =val
                    })
                   let logdata = res;
                ...
                if ( JSON.parse(state).useCache !== undefined && JSON.parse(state).useCache === false  ) {
                    ...
                    transactionres.transType = hexCharCodeToStr(transactionres.input)
                    this.setState({
                        detailData: transactionres,
                        inputData:arr1,
                        transType:  hexCharCodeToStr(transactionres.input)
                    });
                    if(transactionres instanceof Object && transactionres.transType.type === 6) {
                        const recycle_tx1 = await recycle_tx({hash:this.state.detailData.hash,addr:""});
                        if(recycle_tx1) {
    ...
    render() {
        return (
            <>
                <div className={TradeDetail_ls.TradeDetailBox}>
                    ...
                    <div className={TradeDetail_ls.TradeDetailBoxTop}>
                        <div className={TradeDetail_ls.hash}>
                            ...
                            <AiOutlineCopy
                                onClick={handleCopy.bind(this, this.state.detailData.hash)}
                            />
                        </div>
                        <div className={TradeDetail_ls.tags}>
                            <div>
                                <Tag
                                    color={this.state.detailData.status == 1? "rgba(168, 255, 210, .2)" : "rgba(254, 79, 167, .2)"}
                                    style={{
                                        color: this.state.detailData.status == 1? 'rgba(158, 255, 204, 1)' : "#FE4FA7",
                                        padding: "0 15px"
                                    }}
                                >
                                    {this.state.detailData.status == 1? "Success" : "Defeat"}
                                </Tag>
                            </div>
                        ...
                        <div className={TradeDetail_ls.TradeDetailBoxBottom}>
                            ...
                            <div className={TradeDetail_ls.common}>
                                <p>Transaction Block</p> <span><Link to={{  pathname: '/BlockChain/BlockDetails', state: { blockid: this.state.detailData.blockNumber } }} style={{color:'#7AA4FF'}}>{this.state.detailData.blockNumber}</Link></span>
                            </div>
                            <div>
                                <p>TXN Fee</p> <span>{(this.state.detailData.gasPrice * this.state.detailData.gasUsed)/1000000000000000000} ERB</span>
                            </div>
                            <div>
                                <p>TXN Hash</p>
                                <Tooltip
                                ...
                                >
                                    <span style={{ paddingRight: 8 }}>
                                        {this.state.detailData.hash.slice(0, 12) +
                                            '...' +
                                            this.state.detailData.hash.slice(
                                                -16,
                                                this.state.detailData.hash.length,
                                            )}
                                    </span>
                                </Tooltip>
                                <AiOutlineCopy
                            ...
                            <div>
                                <p>Timestamp</p> <span>{batchTime(this.state.detailData.timestamp)}</span>
                            </div>
                            <div>
                                <p>Volume</p> <span>{this.state.detailData.value /1000000000000000000} ERB</span>
                            </div>
                            <div className={TradeDetail_ls.common}>
                                <p>Sender</p>
                                <Tooltip
                                ...
                                >
                                    <span style={{ paddingRight: 8 }} onClick={this.handleRouter.bind(this,"/AccountDetail",this.state.detailData.from)}>
                                        {this.state.detailData.from.slice(0, 12) +
                                            '...' +
                                            this.state.detailData.from.slice(
                                                -16,
                                                this.state.detailData.from.length,
                                            )}
                                    </span>
                                </Tooltip>
                                <AiOutlineCopy
                            ...
                            <div>
                                <p>Gas Price</p>
                                <span>{Number(this.state.detailData.gasPrice/1000000000)} Gwei</span>
                            </div>
                            <div>
                                <p>Gas Used</p>
                                <span>{Number(this.state.detailData.gasUsed).toLocaleString()} gas</span>
                            </div>
                            <div className={TradeDetail_ls.common}>
                                <p>Receiver</p>
                                <Tooltip
                                ...
                                >
                                    <span style={{ paddingRight: 8 }} onClick={this.handleRouter.bind(this,"/AccountDetail",this.state.detailData.to)}>
                                        {
                                            this.state.detailData.to ?
                                            this.state.detailData.to.slice(0, 12) +
                                            '...' +
                                            this.state.detailData.to.slice(
                                                -16,
                                                this.state.detailData.to.length,
                                            ):""}
                                    </span>
                                </Tooltip>
                                {
                                    this.state.detailData.to ? (<AiOutlineCopy
                                        onClick={handleCopy.bind(
                                            this,
                                            this.state.detailData.to,
                                        )}
...

                            {
                                this.state.transType.type === 6 ?
                                    <div>
                                        <p>ERB Got</p>
                                        <span>{Number(this.state.newDetailData.price/1000000000000000000 || 0).toLocaleString()} ERB</span>
                                    </div>
                                    : ""
                            }
                            {
                ...
                <div className={TradeDetail_ls.TradeDetailBox1} style={ this.state.typeGroup === 'log' ? { } : {padding:0} } >
                    {
                        this.state.typeGroup === "log"?
                            <div>
                            <div className={TradeDetail_ls.TradeDetailBox1Title} >
                                <ul>Transaction Ledger</ul>
                                <span>Transaction Receipt Event Logs</span>
                            </div>
                            <div>
                                {this.state.dataLog.length!==0? this.state.dataLog.map((item, index1) => (
                                    <div className={TradeDetail_ls.TradeDetailBox1Logs}>
                                        <ul>
                                            <Avatar
                                                shape={'circle'}
                                                size={56}
                                                style={{
                                                    background: 'rgba(89, 73, 19, 1)',
                                                ...
                                                <ul>
                                                    ...
                                                    <li>
                                                        <div>
                                                            <p>0</p>
                                                            <span>
                                                        {JSON.parse(item.topics[0]).value}
                                                    </span>
                                                        </div>
                                                        {
                                                            item.topics.length > 1 ?
                                                                (
                                                                    item.topics.map((item1,index) => (
                                                                        <div style={{marginTop:"10px;"}} className={TradeDetail_ls.topicsMap}>
                                                                            <p>{index+1}</p>
                                                                            <Select
                                                                                defaultValue={`Hex`}
                                                                                onChange={
                                                                                    this.handleChange.bind(this,index,index,index1)
                                                                                }
                                                                                suffixIcon={
                                                                                    <>
                                                                                        <GoTriangleDown
                                                                            ...
                                                                            <span title={JSON.parse(item1).type ? JSON.parse(item1).value :ex16hex(JSON.parse(item1).value).toLocaleString(18) }>
                                                                    {JSON.parse(item1).type ? JSON.parse(item1).value :ex16hex(JSON.parse(item1).value).toLocaleString(18) }
                                                    </span>
                                                                        </div>
                                                                    ))
                                                                ): ""
                                                    ...
                                                    >
                                                        ...
                                                        <p>
                                                            ...
                                                            'Hex' ? (
                                                                ...
                                                                <span title={ex16hex(item.data).toLocaleString(18)}>{ex16hex(item.data).toLocaleString(18)}</span>
                                                            )}
                                                        </p>
                                                    </div>
                                                </ul>
                                ...
                                )): (<div className={TradeDetail_ls.notData}>No Data</div>)}
                            </div>
                            </div> :
                            <div className={TradeDetail_ls.TradeDetailBox2}>
                                <div style={{padding:"24px",wordWrap:'break-word'}}>
                                    {
                                        !this.state.detailData.to || this.state.detailData.input.length <=10 ?this.state.detailData.input : this.state.detailData.input.slice(0,22) === "0x776f726d686f6c65733a"?this.state.detailData.input:
                                            <div>
                                                {
                                                    knownHashes[this.state.detailData.input.slice(0,10)]?<p style={{pddingBottom: "33px"}}>{"Function: "+ knownHashes[this.state.detailData.input.slice(0,10)]}</p>: (<p></p>)
                                                }
                                                <p>MethodID:{this.state.detailData.input.slice(0,10)}</p>
                                                {this.state.inputData.map((item,index) => (
                                                    <p key={index}><span>[{index}]: </span>{item}</p>
                                                ))}
                                            </div>
